<template>
    <div id="cart-value">
        <div class="inline image">
            <img v-bind:src="this.item.photo_filename" width="auto" height="150">
        </div>
        <div class="inline" style="color: white; width: 40%;vertical-align: middle;text-align: left;">
            {{item.name}} {{item.size ? item.size + ' л' : ''}}
        </div>
        <div style="vertical-align: middle; position:relative;">
            <span v-if="item.discount_price"
                  style="color: #b1b1b1; font-size: 12px; position:absolute; padding-right: 10px; left: -100%; bottom: 0; text-decoration: line-through;">{{item.price}} грн</span>
